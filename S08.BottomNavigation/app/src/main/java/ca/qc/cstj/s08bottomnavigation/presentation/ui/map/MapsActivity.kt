...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(locationRequestPermission.checkStatus().allDenied()){
            Toast.makeText(this,"Permissions non accordée",Toast.LENGTH_SHORT).show()
            finish()
        }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
       ...
       // mMap.isTrafficEnabled = true
        ...
        val cameraOptions = CameraPosition.Builder().target(args.position).zoom(12f).build()

        mMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraOptions))
